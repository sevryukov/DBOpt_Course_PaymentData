    ...
    for _ in range(count):
        ...
        values.append(
            (randint(100, 10**5), choice(category_oids), choice(project_oids),
             ...
             payer_oid, payee_oid, 0, create_date.strftime(date_format),
...
def generate_insert_payments_by_single_queries(values, file_name: str):
    ...
    with open(file_name, 'w', encoding=ENCODING) as sql_file:
        ...
        query_format = (
            ...
            " IsAuthorized, Number) VALUES (NEWID(), %d, '%s', '%s', '%s',"
...
if __name__ == '__main__':
    ...
    generate_insert_payments_by_single_queries(
        values=values,
        file_name=f'InsertBySingleQueries{queries_count}.sql',
    )

    generate_insert_payments_by_batched_queries(
        values=values,
        file_name=f'InsertByBatchedQueries{queries_count}.sql',
    )
